...
<body>
    ...
    <h4>Faculty Information (Update)</h4>
    <br>

    <form action="Faculty_Info.php" method="POST">

        <label for="NSU_Faculty_id">NSU ID: </label>
        <input type="number" name = "NSU_Faculty_id" id = "NSU_Faculty_id" placeholder="Insert NSU ID"  maxlength="10" required> <br>
        ...
        <input type= "text" name = "Faculty_Initial" id = "Faculty_Initial" placeholder="Insert Faculty Initial" required><br>
...
<?php  
    ...
    if(isset($_POST['Submit'])){
        ...
        // for nsu faculty
        $squery2="select * from nsu_faculty where NID='$NID'";
    ...
    if($sresult1){
        ...
        $iquery1 = "insert into faculty( NID, Name, Address, Phone_No, Date_Of_Birth, DoseDate1, DoseDate2) 
    ...
    if($sresult1){
        ...
        // "NSU Faculty" table delet check + delete
            if($sresult2){
                $dquery2 = "delete from nsu_faculty where NID='$NID'";
                $dresult2=mysqli_query($con,$dquery2);
            }
        
        // Normal Student delete
        $dquery1 = "delete from faculty where NID='$NID'";
